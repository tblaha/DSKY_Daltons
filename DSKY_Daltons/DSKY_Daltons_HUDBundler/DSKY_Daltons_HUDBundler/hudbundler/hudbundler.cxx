...
hudbundler::hudbundler(Entity* e, const char* part, const
  ...
  LeaderboardStreamToken(getId(), NameSet("world", LeaderboardStream::classname, part)),
  fuelRewardEventReadToken(getId(), NameSet("world", FuelRewardEvent::classname, part)),
  AnnouncementEventToken(getId(), NameSet("world", AnnouncementEvent::classname, part)),
...
void hudbundler::doCalculation(const TimeSpec& ts)
{
  ...
  HUDWriter.data().terminal_z         = FCReader.data().terminal_z;
  HUDWriter.data().hud_range          = FCReader.data().hud_range;
  ...
  // Leaderboard
  try {
    //StreamReader<LeaderboardStream> LeaderboardReader(LeaderboardStreamToken, ts); // doesn't work
    //StreamReader<LeaderboardStream> LeaderboardReader(LeaderboardStreamToken, ts-200); // sometimes works
    StreamReaderLatest<LeaderboardStream> LeaderboardReader(LeaderboardStreamToken); // works, but no idea for how long
    ...
    W_MOD(classname << ": Error while reading LeaderboardStream channel!" << e);
  }
