...
/** Common base class for WorldViewerBase and ListenerBase, provides
  ...
  */
  ...
  /** Retrieve a specification for a graphical object, previously added by
      ...
      - If the name from a found dataclass ends with a '#' character,
        (either because this was given in the specification, or because
        it was given in the label name, or because it was added after
        not finding a match) the creation id from the channel is added 
        to the object name (for automatic naming). So the name may be
        "B737 #", that will result in "B737 #1", "B737 #5", etc. Note 
        that the numbering is based on the entries in the channel, and 
        entries in-between might match other spec files.
  */
