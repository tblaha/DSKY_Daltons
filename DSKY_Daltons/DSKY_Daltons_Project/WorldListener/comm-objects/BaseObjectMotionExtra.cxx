...
public:
  ...
  typedef const Eigen::Map<const Eigen::VectorXd> cVectorEd;
  typedef Eigen::Map<Eigen::VectorXd> VectorEd;
private:
...
public:
  /** Constructor. Note that d_x is taken as a 7 element
      ...
      \param d_uvw speed in body axes, 3 elt, double.
      \param d_omega rotational speed in body axes, 3 elt, double. */
  BaseObjectMotion_Body(double *d_x, double* f_uvw, double *f_omega) :
...
BaseObjectMotion BaseObjectMotion::getRelative(const BaseObjectMotion& o) const
{
  ...
  { 
    ...
    // convert velocity
    BaseObjectMotion_Body::VectorEd r_uvw(result.uvw, 3);
    ...
    r_uvw = 
      BaseObjectMotion_Body::cVectorEd(o.uvw.ptr(), 3) -
      (R2.transpose() *
      ( R1 *
        ( X * r_xyz +
          BaseObjectMotion_Body::
          cVectorEd(this->uvw.ptr(), 3).cast<const double>() ) )
             ).cast<const double>();
  ...
  BaseObjectMotion_Body::VectorEd omegar(result.omega, 3);
  omegar = (R2.transpose()*
    (R1*BaseObjectMotion_Body::cVectorEd(this->omega, 3).cast<const double>() -
     R2*BaseObjectMotion_Body::cVectorEd(o.omega, 3).cast<const double>())).cast<const double>();
